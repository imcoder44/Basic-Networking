    ...
    def send(self):
        ...
        try:
            while True:
                ...
                # Avoid asking for input if stdin is closed (non-interactive mode)
                if sys.stdin.isatty():
                    buffer = input('')
                    buffer += '\n'
                    self.socket.send(buffer.encode())
                else:
                    break # Exit loop if not interactive
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent('''\
            Examples:
              ...
              netcat.py -t 192.168.1.108 -p 5555 -l -u=my.txt # upload file
              netcat.py -t 192.168.1.108 -p 5555 -l -e="ls"   # execute command
